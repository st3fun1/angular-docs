...
export class FakeApiService {
  ...
  // declarative
  photos$: Observable<Photo[]> = this.http
    .get<Photo[]>(`${this.baseUrl}/${this.entity}`)
    .pipe(
      map((photos: Photo[]) => {
        ...
        photos.forEach((photo) => {
          if (photo.albumId <= 4) {
            if (!listOfPhotos[photo.albumId]) {
              ...
              listOfPhotos[photo.albumId] &&
              listOfPhotos[photo.albumId].length < 3
            ) {
              listOfPhotos[photo.albumId] = [
                ...listOfPhotos[photo.albumId],
                photo,
  ...
  photosWithCategories$: Observable<Photo[]> = combineLatest([
    this.photos$,
          ...
          ({
            ...
            categoryName:
              photoCategories.find(
                (category) => category.categoryId === photo.albumId
              )?.categoryName || 'Random Album',
          } as Photo)
      );
    }),
    shareReplay(1)
  ...
  photosWithAdd$: Observable<Photo[]> = merge(
    this.photosWithCategories$,
