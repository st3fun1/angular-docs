...
import { PhotoCategory } from '../shared/interfaces/photo-category';
...
export class FakeApiService {
  ...
  photos$: Observable<Photo[]> = this.http.get<Photo[]>(`${this.baseUrl}/${this.entity}`).pipe(
    map( (photos: Photo[]) => {
      ...
      return listOfPhotosKeys.map( (key: string) => {
  ...
  photosWithCategories$: Observable<Photo[]> = combineLatest([
    ...
    map(([photos, photoCategories]) => {
      return <Photo[]>(photos.map((photo: Photo) => ({
        ...photo,
